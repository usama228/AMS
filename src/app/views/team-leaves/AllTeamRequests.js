...
import { DELETELEAVEREQUEST, GETALLLEAVES, GETUSERSBYTEAMLEAD, UPDATELEAVESTATUS } from 'app/redux/actions';
import {
    TableBody,
    TableCell,
    TableHead,
...
const AllTeamRequests = () => {
    ...
    useEffect(() => {
        if (employees?.allUsers?.users.length > 0) {
            setSelectedEmployee(employees.allUsers.users[0].id);
        }
    }, [employees]);
    ...
    const handleEditClick = (id) => {
        navigate(PATH.CREATETEAMREQUEST.replace(":id", id));
    };

    const confirmDelete = () => {
        dispatch(DELETELEAVEREQUEST(leaveToDelete, null, onError));
        setDialogOpen(false);
    ...
    return (
        <SimpleCard>
            ...
            <Grid container spacing={2}>
                <Grid item xs={4}>
                    {(auth.userType === 'employee' && auth.isTeamLead === true) && (
                        <TextField1
                            fullWidth
                            select
            ...
            <Grid container spacing={2}>
                <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'end', alignItems: 'center' }}>
                    <Button
                        variant="outlined"
                        onClick={() => navigate(PATH.CREATETEAMREQUEST.replace(":id", "new"))}
                    >
                        Create Leave
                    </Button>
                </Grid>
